...
aspect=pickle.load(open('final_aspect_dump.p', 'rb'))			#aspect_term extractor dictionary
...
def extractor(words = {}, sid=0):				#sid = sentence id, and words = aspect terms 
	...
	for j in words :						# one by one aspect terms theeskuntam
	    ...
	    if len(j.split(' ')) == 1 and j in sentence[sid] :		#then if aspect term is noun go inside
	    	#print sid,sentence[sid]
	    	p =  sentence[sid][j]
	    	print p
		#print 'yipppeeeeeee'
		for i in p:					#iterate through its values
			...
			if wordInSentic(p[i]):			#aspect term tho relate aina each word sentnic ,
				flag=1				#based on that positive, negative, neutral values 
#print i
